...
*/
function colorPicker(e,targetComponentId,mode,size,allowResize,allowClose,allowDrag,expColor,expHEX,offsetX,offsetY,orientation,parentObj,parentXY,color,difPad,rSpeed,cookieLife,docBody){
	
	// attribute definitions
	var	cP = colorPicker, // for now to make code shorter
	targetComponentId = targetComponentId,
	mode = mode||cP.mode||'B', // ['H', 'S', 'V', 'R', 'G', 'B'] initial colorMode
	size = size||cP.size||3, //[1, 2, 3, 4] initial size from XXS to L
	...
	offsetY = offsetY||cP.offsetY||4, // [int] y-offset in pixels relative to initial object
	...
	CP=(!cP)?function(obj) { // building the colorPicker / setting up events and methodes
		...
		cP = colorPicker.cP = document.body.appendChild(div.getElementsByTagName('div')[0]);
	...
	doRender = function(xy,yz) { // function for pure rendering. No rendering elsewhere!!
		...
		if (expHEX) {
				Ext.getCmp(ColorPickerControlSingleton.currentTarget).setValue(CP.valPrefix+CP.hex);
		}
	...
	toggleCp = function(onOff,obj) { 
		...
		if (onOff && !parentObj) cP.cPSkins.display = 'none';
				...
				if (cP.cObj && obj.parentNode != cP.cObj.parentNode) document.body.appendChild(cP.parentNode.removeChild(cP));
