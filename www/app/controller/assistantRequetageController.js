    ...
    adaptToTCSelect: function(keepInMind) {
        ...
        if(!Ext.isEmpty(typesContenus)){
            ...
            if (typesContenus.length==1) {
                ...
                var champsEligibles = ["datefield", "numberfield", "timefield", "slider","ratingField","textfield"];
            ...
            } else if (typesContenus.length>1) {
                var myThingType=Ext.getStore('TCNDepCombo').findRecord('id',typesContenus[0]);
                var champsReq = Ext.clone(myThingType.data.champs);
                var champsEligibles = ["datefield", "numberfield", "timefield", "slider","ratingField","textfield"];
                var champsReqTypes=Ext.Array.pluck(Ext.Array.pluck(champsReq,"config"),"name");
                var jj = 1;
                for (jj=1; jj<typesContenus.length; jj++) {
                    var nextTypeFields = Ext.clone(Ext.getStore('TCNDepCombo').findRecord('id',typesContenus[jj]).data.champs);
                    champsReqTypes = Ext.Array.intersect(champsReqTypes, Ext.Array.pluck(Ext.Array.pluck(nextTypeFields,"config"),"name"));
                }
                var champsReqF = Ext.Array.filter(champsReq, function(champ){
                    if (Ext.Array.contains(champsEligibles, champ.cType)&&Ext.Array.contains(champsReqTypes, champ.config.name)) {return true;} else {return false;}
                });
                champsReqF = Ext.Array.map(champsReqF, function(champ){
                    return ({nom:champ.config.fieldLabel, valeur:{cType: champ.cType, ruleId: "fields."+champ.config.name, name: "fields."+champ.config.name, label: champ.config.fieldLabel}});
                });
